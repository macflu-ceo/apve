...
import { sendPushToGradeName } from "@/lib/push";
...
export async function reserveVisit(couponId: string, date: string, time: string) {
  ...
  const coupon = await prisma.coupon.findUnique({
    where: { id: couponId },
    select: { id: true, status: true, customerName: true, store: { select: { name: true } } },
  });
  ...
  // '매장' 등급(매장 직원)에게 예약 알림 푸시 (등급 없으면 무시)
  await sendPushToGradeName(
    "매장",
    {
      title: "새 방문 예약",
      body: `${coupon.customerName}님 · ${coupon.store.name} ${date} ${time}`,
      url: "/store-scan",
    },
    "reservation"
  ).catch(() => {});
