...
export default async function TimeSalePage() {
  ...
  return (
    <div className="px-4 py-6">
      {/* 히어로 */}
      <div className={`mb-6 rounded-xl2 p-6 text-center text-white ${upcoming ? "bg-gradient-to-br from-[#2b2622] to-[#4a3f36]" : "bg-gradient-to-br from-[#e5322f] to-[#c81e1a]"}`}>
      ...
      {upcoming ? (
        <div className="rounded-xl2 border border-line bg-[#faf9f8] p-10 text-center">
          ...
          <p className="mt-2 text-sm text-sub">
            오픈되면 <b className="text-deal">특별 추가 할인가</b>로 만나보실 수 있어요.
            <br />
            타이머가 끝나면 자동으로 공개됩니다.
          </p>
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-6 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
          {items.map(({ product: p, discount, salePrice, dealPrice }) => {
            const img = parseList(p.imagesJson)[0];
            const commission =
              dealPrice != null ? Math.round((dealPrice * rate.percent) / 100) : null;
            return (
              <Link key={p.id} href={`/goods/${p.goodsNo}`} className="group block">
                <div className="relative aspect-[3/4] overflow-hidden rounded-xl2 bg-[#f5f4f2]">
                  ...
                  <span className="absolute left-2 top-2 rounded-md bg-[#e5322f] px-2 py-1 text-xs font-black text-white">
                    {discount}% ↓
                  </span>
                </div>
                <div className="mt-2">
                  ...
                  <div className="mt-1 flex items-baseline gap-1.5">
                    <span className="text-base font-extrabold text-[#e5322f]">{won(dealPrice)}</span>
                    {salePrice != null && (
                      <span className="text-xs text-sub line-through">{won(salePrice)}</span>
                    )}
                  </div>
                  {commission != null && rate.percent > 0 && (
                    <div className="mt-0.5 text-[11px] text-sub">
                      예상 수수료 <b className="text-brand">{won(commission)}</b>
                    </div>
                  )}
                </div>
              </Link>
