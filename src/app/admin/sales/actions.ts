...
import { notifyNewSales } from "@/lib/crm/notify";
...
export async function syncSalesAction(from: string, to: string) {
  ...
  try {
    ...
    // 새 구매확정 판매에 대해 파트너 알림톡 (mock — 발송대행사 연결 시 실발송)
    const notified = await notifyNewSales();
    ...
    if (notified > 0) parts.push(`판매알림 ${notified}건`);
