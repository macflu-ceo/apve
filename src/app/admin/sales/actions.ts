...
import { runSaleTriggers } from "@/lib/crm/notify";
...
export async function syncSalesAction(from: string, to: string) {
  ...
  try {
    ...
    // 알림톡 트리거: 판매발생 + 수수료 마일스톤 (mock — 발송대행사 연결 시 실발송)
    const t = await runSaleTriggers();
    ...
    if (t.sale + t.milestone > 0) parts.push(`알림톡 ${t.sale + t.milestone}건`);
